            ...
            <div class="container b-radius-top">
                ...
                <div class="main">
					<div class="main-item">
						...
						<%
							ArrayList<News> arSearching = (ArrayList<News>) request.getAttribute("arSearching");
							if(arSearching != null && arSearching.size() > 0){
								for(int i = 0 ; i < arSearching.size() ; i++){
						%>
							<div class="main-news">
								<a href="<%=request.getContextPath()%>/<%=StringLibrary.createSlug(arSearching.get(i).getName()) %>/detail/<%=arSearching.get(i).getId()%>"><img src="<%=request.getContextPath() %>/images/<%=arSearching.get(i).getPicture() %>" class="img-responsive" alt=""></a>
								<h3><a href="<%=request.getContextPath()%>/<%=StringLibrary.createSlug(arSearching.get(i).getName()) %>/detail/<%=arSearching.get(i).getId()%>"><%=arSearching.get(i).getName() %></a></h3>
								<p><%=arSearching.get(i).getPreview() %></p>	
							</div>
						<%}}else{%>
							<p style="font-weight:bold ; font-size:30px;">Không tìm thấy tin nào</p>
						<%}%>
